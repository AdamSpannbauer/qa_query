...
class NasdaqScraper(Scraper):
    ...
    splash_page_n_url = (
        "https://www.nasdaq.com/news-and-insights/topic/markets/page/{n}"
    )
    ...
    def _scrape_article_urls(self, page=1):
        ...
        for article_div in article_divs:
            ...
            article_url = (
                ...
                + article_a_tag.get_attribute_list("href")[0].strip()
            )
    ...
    def scrape_articles(self, output_dir, n_pages, page_offset=0):
        ...
        for i, article_url in enumerate(self.article_urls):
            ...
            article_published_datetime = article_soup.select(".timestamp__date")[0][
                "datetime"
            ]
            article_published_datetime = dateutil.parser.parse(
                article_published_datetime
            )

            article_p_tags = article_soup.select(self.article_text_selector)
            article_text = [
                p_tag.text for p_tag in article_p_tags if p_tag.text.strip()
            ]
            ...
            article_dict = {
                ...
                "title": article_title,
                "text": article_text,
                "published_datetime": article_published_datetime.strftime(
                    "%Y-%m-%d %H:%M:%S"
                ),
            }

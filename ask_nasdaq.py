...
def format_answer(question_text, answer_list, search_result, context_pad_size=10):
    ...
    context_end = min(
        [len(search_result["article"]), answer_start_idx + context_pad_size]
    )
...
def answer_display_df(
    answer_df,
    n_answers=3,
    rank_by="combined_rank",
    display_fields=None,
    display_names=None,
):
...
def qa_query_nasdaq(
    ...
    for i, search_result in enumerate(search_results):
        full_answer = bert_squad_inst.ask_question(
            search_result["article"], question_text
        )
    ...
    answer_df["combined_rank"] = (
        answer_df["squad_rank"] + answer_df["search_rank"]
    ).rank()
...
if __name__ == "__main__":
    ...
    ap.add_argument(
        "-a", "--n_answers", default=10, type=int, help="Number of answers to print"
    )
    ap.add_argument(
        "-s",
        "--n_search_results",
    ...
    ap.add_argument("-n", "--index_name", default="nasdaq", help="Name of whoosh index")
    ...
    query_parser = MultifieldParser(
        search_fields, schema=whoosh_idx.schema, group=OrGroup
    )
    ...
    with whoosh_idx.searcher() as searcher:
        while True:  # To infinity and beyond
            ...
            nasdaq_answer_df = qa_query_nasdaq(
                question,
                query_parser,
                searcher,
                bert_squad,
